...
def plot_subfig(dataset, f, ax, ax1, ax2, legend=False, include_xlabel=True, add_to_legend='', include_ylabel=True):
    ...
    if sparsity_metric == 'default': 
        ...
        s_size_cutoff = 25
    if dataset == 'PHARYNGITIS' or dataset == 'HEART_DISEASE': 
        s_size_cutoff = 72.5

    window_cutoff = 160 if 'BREAST_CANCER' not in dataset else 90
    window_cutoff = 80 if 'PHARYNGITIS' in dataset else window_cutoff
    y_lim=(0.7, 0.87) if 'PHARYNGITIS' not in dataset else (0.62, 0.82)
    ...
    if 'BREAST_CANCER' not in dataset: 
    ...
    if include_xlabel: 
        f.text(0.5, -0.04, x_title, ha='center')

    


f_overall = plt.figure(layout='constrained', figsize=(20, 10))
...
for i, dataset in enumerate(datasets): 
    ...
    if i == 0: 
        subfigs[0].suptitle('Rate $\leq$ 0.25', x=0.57)
        subfigs[1].suptitle('Rate $\leq$ 0.5', x=0.57)
...
f_overall.suptitle('Performance under Added Synthetic MAR Missingness', fontsize= 'x-large')
legend = f_overall.legend(loc='lower center', fancybox=True, shadow=True, ncol=4, bbox_to_anchor=(0.5, -0.16))
for i in [0, 1]:
