...
def plot_subfig(dataset, f, ax, ax1, ax2, legend=False, include_xlabel=True):
    ...
    uncertainty_bands_subplot(sparsity_aug, train_auc_aug, 'M-GAM (w/ Interactions)' if legend else None, ax2)
...
f_overall.suptitle('Test Performance vs Sparsity', fontsize= 'xx-large')
legend = f_overall.legend(loc='lower center', fancybox=True, shadow=True, ncol=4, bbox_to_anchor=(0.5, -0.12))#-0.18))

for i in [0, 1]:
    legend.get_texts()[i].set_weight('bold')

plt.savefig(f'./figs/test4{datasets}.{filetype}', bbox_inches='tight')

print('successfully finished execution')
print(f'./figs/test4{datasets}.{filetype}')
