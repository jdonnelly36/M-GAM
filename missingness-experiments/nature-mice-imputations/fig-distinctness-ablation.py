...
parser.add_argument('-w', '--subfig_width', type=int, default=5, help='Number of subfigures per col')
args = parser.parse_args()

datasets = ['distinctness_False_False_True_True', 'distinctness_True_False_True_True', 
            'distinctness_False_True_True_True', 'distinctness_True_True_False_False', 
            'distinctness_True_True_True_True', #'distinctness_True_False_True_False',
            'distinctness_False_True_True_False', 'distinctness_True_True_True_False',
            'distinctness_True_True_False_True', 'distinctness_True_False_False_True',
]
...
DATASET_NAME = {
    ...
    'distinctness_False_False_True_True': 'Indicator: Specific \n Interaction: Specific', 
    'distinctness_True_False_True_True': 'Indicator: Specific & Overall \n Interaction: Specific',
    'distinctness_False_True_True_True': 'Indicator: Specific \n Interaction: Specific & Overall',
    'distinctness_True_True_True_True': 'Indicator: Specific & Overall \n Interaction: Specific & Overall',
    'distinctness_True_True_False_False': 'Indicator: Overall \n Interaction: Overall',
    # 'distinctness_True_False_True_False': 'Indicator: Specific & Overall \n Interaction: None',
    'distinctness_False_True_True_False': 'Indicator: Specific \n Interaction: Overall',
    'distinctness_True_True_True_False': 'Indicator: Specific & Overall \n Interaction: Overall',
    'distinctness_True_False_False_True': 'Indicator: Overall \n Interaction: Specific',
    'distinctness_True_True_False_True': 'Indicator: Overall \n Interaction: Specific & Overall',
}
...
def plot_subfig(dataset, f, ax, ax1, ax2, legend=False, include_xlabel=True):
    ...
    res_dir = f'experiment_data/FICO'
    res_dir = f'{res_dir}/{dataset}'
    ...
    for imputer in baseline_imputers: 
        ...
        imp_res_dir = f'experiment_data/FICO/noreg/distinctness_{overall_mi_intercept}_{overall_mi_ixn}_{specific_mi_intercept}_{specific_mi_ixn}/impute_{imputer}/100'+(
    ...
    ax.set_ylim(*y_lim)
    if False:# in dataset and 'HEART_DISEASE' not in dataset: 
        ax1.set_ylim(*y_lim)
        if 'MIMIC' in dataset: 
...
subfigs_overall = f_overall.subfigures(subfig_width, int(np.ceil(len(datasets)/subfig_width)))
...
for i, dataset in enumerate(datasets): 
    ...
    if False:# in dataset and 'HEART_DISEASE' not in dataset: 
        ...
        plot_subfig(dataset, subfig, axs[0], axs[1], axs[2], legend=(i==(len(datasets)-1)), include_xlabel=((i % subfig_width) == subfig_width - 1))
    else: 
        axs = subfig.subplots(1, 2, sharey=True, gridspec_kw={'width_ratios': [2, 1]})
        plot_subfig(dataset, subfig, axs[0], None, axs[1], legend=(i==(len(datasets)-1)), include_xlabel=((i % subfig_width) == subfig_width - 1))
    subfig.suptitle(DATASET_NAME[dataset], x=0.57)

f_overall.suptitle('Test Performance vs Sparsity for Distinct Missingness Encodings', fontsize= 'xx-large')
f_overall.legend(loc='lower center', fancybox=True, shadow=True, ncol=4, bbox_to_anchor=(0.5, -0.22))

plt.savefig(f'./figs/appendix/distinct.{filetype}', bbox_inches='tight')

print('successfully finished execution')
print(f'./figs/appendix/distinct.{filetype}')
