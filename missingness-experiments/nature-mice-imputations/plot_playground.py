...
def load_train_test_sparsity(method='SMIM', aug_level = 0, use_mean=True): 
    ...
    return (
        np.loadtxt(f'{m_res_dir}/train_acc_indicator.csv'),
        np.loadtxt(f'{m_res_dir}/test_acc_indicator.csv'),
...
def plot_with_break(is_train = True): 
        ...
        uncertainty_bands_subplot(noreg_sparsity, noreg_train_perf if is_train else noreg_test_perf, 'GAM', ax)# w/ MVI
