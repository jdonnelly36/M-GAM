...
parser.add_argument('-d', '--datasets', nargs='+', type=str, default='FICO', help='dataset name')
parser.add_argument('-f', '--filetype', type=str, default='png', help='Filetype for saving the figure')
parser.add_argument('-x', type=int, default=12, help='Width of the figure')
parser.add_argument('-y', type=int, default=6, help='Height of the figure')
...
sparsity_metric = 'num_variables'# 'default', 'num_variables'
...
def plot_subfig(dataset, f, ax, ax1, ax2, legend=False, include_xlabel=True, num_quantiles=8):
    ...
    def load_train_test_sparsity(method='SMIM', aug_level = 0, use_mean=True): 
        ...
        if sparsity_metric != 'default': 
            m_res_dir = f'{m_res_dir}/sparsity_{sparsity_metric}'
        return (
...
f_overall.suptitle('Test Performance vs # Nonzero Shape Functions', fontsize= 'large')
f_overall.legend(loc='lower center', fancybox=True, shadow=True, ncol=4, bbox_to_anchor=(0.5, -0.17))

plt.savefig(f'./figs/appendix/sparsity.{filetype}', bbox_inches='tight')

print('successfully finished execution')
print(f'./figs/appendix/sparsity.{filetype}')
