    ...
    for model_index, model_initializer in enumerate(model_types):
        for holdout_set in holdouts:
            ...
            if 'xgboost' in str(model_initializer):
                clf = GridSearchCV(estimator=model_initializer(error_score='raise'), param_grid=param_grids[model_index], scoring=val_metric)
...
if __name__ == '__main__':
    ...
    imputations = 10
    '''for imputation_method in ['Mean', 'MissForest', 'MIWAE', 'GAIN']:
        for ds_name in ['FICO']:#:'BREAST_CANCER_0.25', 'BREAST_CANCER_0.5', 'BREAST_CANCER_0.75']: #['BREAST_CANCER', ]:
            ...
            if False and 'FICO' == ds_name:
                dataset = f'{dataset_base_path}/DATA_REDUCED/{ds_name}/distinct-missingness/'#'BREAST_CANCER'
            else:
            ...
            for train_rate in [0]:
                for test_rate in [0]:
                    ...
                    if slurm_iter is None:
                        ...
                        res_df = pd.DataFrame()
                        get_timing_and_accuracy_baselines([model_types[slurm_iter // len(holdouts)]], 
    ...
    '''for imputation_method in ['MICE']:#, 'Mean', 'MissForest', 'MIWAE', 'GAIN']:
        for ds_name in ['FICO']:#, 'FICO']:#:'BREAST_CANCER_0.25', 'BREAST_CANCER_0.5', 'BREAST_CANCER_0.75']: #['BREAST_CANCER', ]:
            print(f"Running for {imputation_method}, {ds_name}")
            #for ds_name in ['BREAST_CANCER', 'BREAST_CANCER_0.25', 'BREAST_CANCER_0.5', 'BREAST_CANCER_0.75']:
            if False and 'FICO' == ds_name:
                dataset = f'{dataset_base_path}/DATA_REDUCED/{ds_name}/distinct-missingness/'#'BREAST_CANCER'
            ...
            for train_rate in [0]:
                for test_rate in [0]:
                    ...
                    if slurm_iter is None:
                        ...
                        overall_df.to_csv(f'./parallelized_results/baselines_{date.today()}_iter_{slurm_iter}_{imputations}_imp_all_50_max_coef.csv',index=False)'''
    '''for imputations in [1, 5, 10, 20, 30]:
        for imputation_method in ['MICE']:#, 'Mean', 'MissForest', 'MIWAE', 'GAIN']:
            for ds_name in ['FICO']:#:'BREAST_CANCER_0.25', 'BREAST_CANCER_0.5', 'BREAST_CANCER_0.75']: #['BREAST_CANCER', ]:
                print(f"Running for {imputation_method}, {ds_name}")
                #for ds_name in ['BREAST_CANCER', 'BREAST_CANCER_0.25', 'BREAST_CANCER_0.5', 'BREAST_CANCER_0.75']:
            ...
            for train_rate in [0]:
                for test_rate in [0]:
                    ...
                    if slurm_iter is None:
                        ...
                        res_df = pd.DataFrame()
                        res_df = get_timing_and_accuracy_baselines([model_types[slurm_iter // len(holdouts)]], 
                                    [param_grids[slurm_iter // len(holdouts)]], dataset, dataset_imp, dataset_suffix,
                                    dataset_name=f'{ds_name}', imputation_method=imputation_method, 
                                    imputations=imputations, holdouts=[holdouts[slurm_iter % len(holdouts)]],
                                    val_metric='roc_auc' if 'BREAST' in ds_name else 'accuracy')
